 ...
 * @var array
 */
	public $uses = array('Color', 'Vote'); 
	 public $components = array('Paginator','RequestHandler');
	public $helpers = array('Js' => array('Jquery'));
        public function getvotecount($color_id = null){
                if(!$color_id){
                        throw new NotFoundException(__('Invalid color'));
                }
		$this->Vote->virtualFields['totalVotes']= 'IFNULL(SUM(Vote.votes),0)';
		$votes = $this->Vote->find('first', array(
				'conditions'=>array('Color.'.$this->Color->primaryKey => $color_id),
				'fields'=>array('totalVotes')
				
			));
		
		$this->set('votes', $votes);
        	$this->set('_serialize', array('votes'));	
        }
